...
function avg (arr: Array<number>): number | null {
  const result = arr.length === 0? null : Math.round(
    arr.reduce((total, value) => total + value) / arr.length
    );
  return result;
}
...
function checkMedicine (medicineObj: MedicineObj | null): any {
  return (
    medicineObj === null? null : (
      <div>
        <span>
          아침: { medicineObj.breakfast ? 'O' : 'X' }
        </span>
        <span>
          점심: { medicineObj.lunch ? 'O' : 'X' }
        </span>
        <span>
          저녁: { medicineObj.dinner ? 'O' : 'X' }
        </span>
      </div>
    )
  );
...
function Summary(props: Props) {
  const { beforeArr, afterArr, lowArr, highArr, medicineObj, mealObj } = props.healthInfo;
